...
function generateUUID() {
    ...
    // Fallback para navegadores más antiguos o entornos no seguros
...
const elements = {
    ...
    reports: {
        ...
        printAdicionalesReportBtn: document.getElementById('print-adicionales-report-btn'),
        ...
        generateInstitutionalAdicionalesReportBtn: document.getElementById('generate-institutional-adicionales-report-btn'),
    ...
    settings: {
        ...
        tasksReportBtn: document.getElementById('tasks-report-btn'),
...
function recalculateLocationCounts() {
    state.locations = {}; // Reinicia el contador
...
function handleModalNavigation(modalElement) {
    ...
    const keydownHandler = (e) => {
        if (e.key === 'Tab') {
            ...
            const confirmBtn = modalElement.querySelector('#modal-confirm, #note-save-btn, #edit-adicional-save-btn, #edit-user-save-btn');
            if (confirmBtn && document.activeElement !== confirmBtn) {
                e.preventDefault();
                confirmBtn.click();
            }
        } else if (e.key === 'Escape') {
            const cancelBtn = modalElement.querySelector('#modal-cancel, #note-cancel-btn, #photo-close-btn, #edit-adicional-cancel-btn, #edit-user-cancel-btn, #log-close-btn');
...
function showConfirmationModal(title, text, onConfirm, options = {}) {
    ...
    elements.modalCancelBtn.style.display = ''; // Resetea la visibilidad del botón de cancelar
...
function saveState() {
    if (state.readOnlyMode) return; // No intentar guardar si ya estamos en modo de solo lectura
    ...
    try {
        ...
        // Activamos el modo de emergencia de solo lectura
        state.readOnlyMode = true;
        
        // Mostramos una advertencia persistente y bloqueamos la UI
        checkReadOnlyMode(); 

        // Mostramos una alerta modal para asegurar que el usuario se entere
        showConfirmationModal(
            ...
            () => {}, // No hacemos nada al confirmar, solo es informativo
            { confirmText: 'Entendido', cancelText: '' } // Solo un botón de confirmación
        );
        
        // Ocultamos el botón de cancelar si existe
        if(elements.modalCancelBtn) elements.modalCancelBtn.style.display = 'none';

        // Detenemos el autoguardado para no repetir el error
...
// --- GESTIÓN DE LA INTERFAZ ---
    ...
    if (state.readOnlyMode) {
        ...
        document.querySelectorAll(`
            ...
            .save-new-clave-btn, .new-clave-input
        `).forEach(el => {
            el.disabled = true;
...
function highlightText(text, searchTerm) {
    ...
    // Escapa caracteres especiales en el término de búsqueda para usarlo en la RegExp
    const escapedTerm = searchTerm.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    // Usamos una Expresión Regular para buscar todas las coincidencias, ignorando mayúsculas/minúsculas
...
function renderUserList() {
    ...
    filteredUsers.forEach((user) => {
        // Buscamos el índice original para que los botones de editar/eliminar sigan funcionando
...
// --- BIENES ADICIONALES ---
// --- FUNCIÓN MODIFICADA PARA EL CONTADOR ---
function renderAdicionalesList() {
    ...
    // --- LÍNEA MODIFICADA ---
    // Ahora muestra "visibles de total" en lugar de solo el total.
...
function showPhotoModal(type, id) {
    ...
    // Ocultar todos los contenedores principales al inicio
    viewContainer.classList.add('hidden');
    uploadContainer.classList.add('hidden');
    cameraViewContainer.classList.add('hidden');
    stopCamera(); // Asegurarse de que la cámara se apague si estaba activa

    if (photoExists) {
        // SI HAY FOTO: Mostrar solo el visor de la foto y el botón de eliminar.
        viewContainer.classList.remove('hidden');
        // Intentar cargar la imagen desde la base de datos
        photoDB.getItem(`${type}-${id}`).then(imageBlob => {
            if (imageBlob) {
                const objectURL = URL.createObjectURL(imageBlob);
                img.src = objectURL;
                img.onload = () => URL.revokeObjectURL(objectURL); // Limpiar memoria
            } else {
                img.src = '';
        ...
        // NO HAY FOTO: Mostrar las opciones para subir o capturar una.
        if (!state.readOnlyMode) {
...
function showNotesModal(clave) {
    ...
    // Lógica actualizada para manejar uno o varios bienes
    if (selectedClaves.length > 1) {
        elements.noteTextarea.value = ''; // Si hay varios, empieza con una nota en blanco
        elements.noteTextarea.placeholder = `Añadir una nota a los ${selectedClaves.length} bienes seleccionados...`;
    } else {
        elements.noteTextarea.value = state.notes[selectedClaves[0]] || ''; // Comportamiento original si solo hay uno
...
function exportLabelsToXLSX() {
    ...
    logActivity('Exportación XLSX', `Exportando ${itemsToLabel.length} etiquetas.`);
...
function renderReportStats() {
    ...
    function generateSimplePendingReport() {
        ...
        const areaDisplay = selectedArea !== 'all' 
            ? `${selectedArea} - ${state.areaNames[selectedArea] || 'Área Desconocida'}` 
            : 'Todas las Áreas';
        ...
        printWindow.document.write(`<div class="signatures">
            <div class="signature-box"><p class="signature-line"></p><strong>${state.currentUser.name}</strong><br>Realizó Inventario</div>
            <div class="signature-box"><p class="signature-line"></p><strong>Firma de Recepción</strong><br>Recibe Copia de Pendientes</div>
        </div>`);
...
function generatePrintableResguardo(title, user, items, isAdicional = false) {
    ...
    if (items.length === 0) {
        return showToast(`No se encontraron bienes para el filtro seleccionado.`, 'info');
    }
    ...
    const areaId = isAdicional ? elements.adicionales.areaFilter.value : userData?.area;
    const areaResponsible = areaId && state.areaDirectory[areaId] ? state.areaDirectory[areaId] : { name: '_________________________', title: ''};
    ...
    const responsibleName = (isAdicional && areaId !== 'all') ? `el área ${areaId}` : user;
    ...
    printWindow.document.write(`<div class="signatures">
        <div class="signature-box"><p class="signature-line"></p><strong>${areaResponsible.name}</strong><br>${areaResponsible.title || 'Responsable de Área'}</div>
        <div class="signature-box"><p class="signature-line"></p><strong>${user !== 'all' ? user : 'Firma del Responsable'}</strong><br>Firma de Conformidad</div>
    </div>`);
    printWindow.document.write('</body></html>');
...
function exportInventoryToXLSX() {
    ...
    logActivity('Exportación XLSX', 'Exportando inventario completo.');
...
function generateInstitutionalAdicionalesReport() {
...
function initialize() {
    ...
    elements.logo.title.addEventListener('click', () => {
        logoClickCount++;
        if (logoClickCount === 5) {
            elements.log.showBtn.style.display = 'block';
            showToast('Botón de Registro de Actividad desbloqueado.');
            elements.log.showBtn.click();
            logoClickCount = 0;
        }
    });
    ...
    // --- LISTENER MODIFICADO PARA CORRECCIÓN ---
    elements.userForm.list.addEventListener('click', e => {
        ...
        if (button.classList.contains('activate-user-btn')) {
            ...
            recalculateLocationCounts(); // LLAMADA A LA NUEVA FUNCIÓN
            ...
            showUndoToast('Usuario eliminado.', () => {
                ...
                recalculateLocationCounts(); // LLAMADA TAMBIÉN EN EL UNDO
    ...
    elements.adicionales.printResguardoBtn.addEventListener('click', () => {
        ...
        if (area !== 'all') {
            ...
            reportTitle = user === 'all' ? `el Área ${area}` : user;
        }
        if (user !== 'all') {
    ...
    elements.adicionales.list.addEventListener('click', e => {
        ...
        if (editBtn) showEditAdicionalModal(editBtn.dataset.id);
        if (deleteBtn) {
            if (state.readOnlyMode) return;
        ...
        if (photoBtn) showPhotoModal('additional', photoBtn.dataset.id);
    });
    ...
    const reportButtonConfig = {
        'generate-labels-report-btn': { title: 'Reporte de Etiquetas para Imprimir', filter: item => item['IMPRIMIR ETIQUETA'] === 'SI', withCheckboxes: true, reportType: 'labels', cols: ['Acción', 'Clave Única', 'Descripción', 'Usuario'] },
        'generate-pending-report-btn': { title: 'Reporte de Bienes Pendientes', filter: item => item.UBICADO !== 'SI', withCheckboxes: false, cols: ['Clave Única', 'Descripción', 'Serie', 'Área Original'] },
        'generate-notes-report-btn': { title: 'Reporte de Notas', filter: item => state.notes[item['CLAVE UNICA']], withCheckboxes: true, reportType: 'notes', cols: ['Acción', 'Clave Única', 'Descripción', 'Nota'] },
        'generate-mismatched-area-report-btn': { title: 'Reporte de Bienes Fuera de Área', filter: item => item.areaIncorrecta, withCheckboxes: true, reportType: 'mismatched', cols: ['Acción', 'Clave Única', 'Descripción', 'Área Original', 'Usuario/Área Actual'] }
    };

    Object.keys(reportButtonConfig).forEach(btnId => {
        const buttonElement = document.getElementById(btnId);
        if (buttonElement) {
            buttonElement.addEventListener('click', () => {
                const config = reportButtonConfig[btnId];
                const areaFilter = elements.reports.areaFilter.value;
                const userFilter = elements.reports.userFilter.value;
                
                let filtered = state.inventory.filter(config.filter);

                if(areaFilter !== 'all') {
                    filtered = filtered.filter(item => item.areaOriginal === areaFilter);
                }
                if(userFilter !== 'all') {
                    filtered = filtered.filter(item => item['NOMBRE DE USUARIO'] === userFilter);
                }
                
                renderReportTable(filtered, config.title, { withCheckboxes: config.withCheckboxes, headers: config.cols, reportType: config.reportType });
                showToast(`Reporte de ${config.title} generado con ${filtered.length} bienes.`);
            });
        }
    });

    elements.reports.generateInstitutionalAdicionalesReportBtn.addEventListener('click', generateInstitutionalAdicionalesReport);
    ...
    elements.reports.printResguardoBtn.addEventListener('click', () => {
        ...
        if (!user || user === 'all') {
            return showToast('Por favor, selecciona un usuario para generar el resguardo.', 'error');
        }
        const userItems = state.inventory.filter(item => item['NOMBRE DE USUARIO'] === user);
        if (userItems.length === 0) {
            return showToast(`El usuario ${user} no tiene bienes asignados para generar un resguardo.`, 'info');
        }
        generatePrintableResguardo('Resguardo de Bienes Individual', user, userItems, false);
    });

    elements.reports.printAdicionalesReportBtn.addEventListener('click', () => {
        const user = elements.reports.userFilter.value;
        const area = elements.reports.areaFilter.value;
        let itemsToPrint = state.additionalItems;
        let reportTitle = user;
    ...
    elements.settings.tasksReportBtn.addEventListener('click', () => generateTasksReport());
    ...
    // --- Lógica de Verificación de Duplicados en Tiempo Real ---
    ...
    }, 400)); // Espera 400ms después de que el usuario deja de teclear
    ...
    inventoryTableBody.addEventListener('mouseover', async (e) => {
        ...
        if (state.photos[clave]) {
            ...
            photoPreviewImg.src = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"; // 1x1 transparent gif
            
            // Posiciona el popover cerca del cursor para que aparezca rápido
            photoPreviewPopover.style.left = `${e.pageX + 15}px`;
            photoPreviewPopover.style.top = `${e.pageY + 15}px`;
            photoPreviewPopover.classList.remove('hidden');

            try {
                const imageBlob = await photoDB.getItem(`inventory-${clave}`);
                if (!imageBlob) {
    ...
    inventoryTableBody.addEventListener('mouseout', (e) => {
        ...
        photoPreviewPopover.classList.add('hidden');
        photoPreviewImg.src = ''; // Limpia la imagen para detener la carga si estaba en proceso
        if (currentPhotoUrl) {
            URL.revokeObjectURL(currentPhotoUrl); // Importante: Libera la memoria
            currentPhotoUrl = null;
        }
    });
    // --- Fin de la Lógica de Previsualización ---

    // --- Conectar Buscador de Usuarios ---
    ...
    // --- Lógica para Importador de Fotos Dedicado ---
    ...
    importPhotosInput.addEventListener('change', async (e) => {
        ...
        if (successCount > 0) {
            saveState();
            filterAndRenderInventory(); // Para refrescar los íconos de cámara en la tabla
    ...
    // --- Fin de la Lógica del Importador de Fotos ---

    // --- Lógica para Restaurar Fotos desde un Backup .zip ---
    ...
    restorePhotosInput.addEventListener('change', async (e) => {
        ...
        try {
            ...
            for (let i = 0; i < totalPhotos; i++) {
                ...
                const key = photoFile.name.split('/').pop(); // Ej: 'inventory-41123'
            ...
            if (successCount > 0) {
                saveState();
                filterAndRenderInventory(); // Actualiza la UI
    ...
    // --- Fin de la Lógica de Restauración de Fotos ---
